...
MODELS_DIR = "./ViT/models"
TEST_DIR = "./data/frames"
OUTPUT_DIR = "./ViT/out"
TEST_LABELS_PATH = "./data/labels.csv"
...
def read_frames(video_path, videos):
    ...
    if "Original" in video_path:
        ...
        label = test_df.loc[test_df['filename'] == video_key]['label']
    else:
        label = 1.
...
def sort_tuples_by_filename(tuples, reverse=False):
    sorted_tuples = sorted(tuples, key=lambda x: x[0], reverse=reverse)
    return sorted_tuples


def split_tuple(tuple_list):
    list1, list2 = zip(*tuple_list)
    return list(list1), list(list2)


def get_video_names(video_paths):
    video_names = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default="./ViT/configs/architecture.yaml",
    ...
    if opt.dataset != "DFDC":
        ...
        folders = ['0'] #TODO all folders

    # Read all videos paths
    for folder in folders:
        method_folder = TEST_DIR 
    ...
    predictions = custom_round(np.asarray(preds))
    video_names = get_video_names(video_names)
    merged_list = merge(video_names, predictions)
    sorted_tuple = sort_tuples_by_filename(merged_list)
    video_names, predictions = split_tuple(sorted_tuple)
    submission_df = pd.DataFrame({"filename": video_names, "label": predictions})
    submission_df.to_csv(os.path.join(OUTPUT_DIR, "submission_vit.csv"), index=False)
